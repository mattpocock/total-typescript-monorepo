// http://localhost:3004/courses/exercises/e6f33dcb-c23e-4505-93ff-6b1a84e8c3b0/edit
...
const authedProcedure = loggedProcedure.use(async (opts) => {
  const { ctx } = opts;
...
const organisationProcedure = authedProcedure.use(async (opts) => {
  const { ctx } = opts;
