...
export const processInformationRequests = () => {
  return Effect.gen(function* () {
    if (yield* doesQueueLockfileExist()) {
      return yield* Console.log("⏸️  Queue is locked, skipping information requests");
    }
    ...
    for (const queueItem of informationRequests) {
      ...
      if (queueItem.action.type === "links-request") {
        yield* Console.log(`🔗 Processing links request (${processedRequests}/${informationRequests.length})`);

        const links: { description: string; url: string }[] = [];
        
        yield* Console.log(`📝 Please provide URLs for ${queueItem.action.linkRequests.length} link request(s):`);
        ...
        yield* Console.log(`✅ Links request completed - added ${links.length} link(s)`);
      } else if (queueItem.action.type === "code-request") {
        yield* Console.log(`💻 Processing code request (${processedRequests}/${informationRequests.length})`);
            ...
            codeContent = yield* fs.readFileString(actualCodePath).pipe(
              Effect.catchAll((error) => {
                return Effect.gen(function* () {
                  ...
                  yield* Console.log(`💡 Tip: Check file permissions and ensure the path is correct`);
                  return "";
                });
              })
            );
            yield* Console.log(`✅ Code file loaded: ${actualCodePath} (${codeContent.length} characters)`);
          } else {
            yield* Console.log(
              `⚠️  Warning: Code file ${actualCodePath} does not exist`
            );
            yield* Console.log(`💡 Continuing without code - you can manually add code examples to the article later`);
          }
        } else {
    ...
    yield* Console.log(`🎉 All ${processedRequests} information request(s) processed successfully!`);
  }).pipe(
    Effect.ensuring(
    ...
    while (true) {
      ...
      if (!queueItem) {
        if (processedCount === 0) {
          ...
          yield* Console.log(`✅ Queue processing complete. Processed ${processedCount} item(s)`);
        }
        return;
      }

      processedCount++;
      yield* Console.log(`📦 Processing queue item ${processedCount}: ${queueItem.action.type} (ID: ${queueItem.id})`);

      switch (queueItem.action.type) {
        case "create-auto-edited-video":
          yield* Console.log(`🎬 Creating auto-edited video: ${queueItem.action.videoName}`);
          ...
          if (Either.isLeft(result)) {
            const errorMessage = result.left instanceof Error 
              ? result.left.message 
              : String(result.left);
          ...
          yield* Console.log(`✅ Video creation completed in ${duration}s: ${queueItem.action.videoName}`);
        ...
        case "analyze-transcript-for-links":
          ...
          if (Either.isLeft(transcriptAnalysisResult)) {
            const errorMessage = transcriptAnalysisResult.left instanceof Error
              ? transcriptAnalysisResult.left.message
              : String(transcriptAnalysisResult.left);
              
            yield* Console.log(`❌ Transcript analysis failed: ${errorMessage}`);
            yield* Console.log(`💡 Tip: You can continue with manual article generation using: pnpm cli article-from-transcript`);
          ...
          const linkCount = Array.isArray(transcriptAnalysisResult.right) ? transcriptAnalysisResult.right.length : 0;
          yield* Console.log(`✅ Transcript analysis completed in ${analysisDuration}s. Generated ${linkCount} link request(s)`);
        ...
        case "generate-article-from-transcript":
          ...
          if (Either.isLeft(articleGenerationResult)) {
            const errorMessage = articleGenerationResult.left instanceof Error
            ...
            yield* Console.log(`💡 Tip: Video processing completed successfully. You can manually create an article using: pnpm cli article-from-transcript`);
          ...
          yield* Console.log(`✅ Article generation completed in ${articleDuration}s`);
